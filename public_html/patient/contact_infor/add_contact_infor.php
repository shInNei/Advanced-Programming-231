...
<body>
    ...
    <div>
        <p class = "header"> INPUT CONTACT INFORMATION</p>
    </div>


    <div class = "container1">
        <form action = "add_contact_infor.php" method = "post">
    ...
    <?php       
        if(isset($_POST['Submit'])){
            ...
            if(empty($_POST['birthday']) || empty($_POST['age']) || empty($_POST['job']) || empty($_POST['duration']) || empty($_POST['family_phone']) || empty($_POST['address']) || empty($_POST['history_sick']) || empty($_POST['allergy']) || empty($_POST['symptom'])) 
                ...
                echo 'alert("' . $message . '");'; 
                echo 'window.location.href = "add_contact_infor.php";';
                echo '</script>';}
            else{
                $user = $_SESSION['user'];
                $patient_id = $user['ID'];
                ...
                $sql = "INSERT INTO contact_and_medhis (birthday, age, job, duration_insurance, family_phone, address, id_patient, symptom, history_sick, allergy)
